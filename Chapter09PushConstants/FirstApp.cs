...
public class FirstApp : IDisposable
{
    ...
    private unsafe void recordCommandBuffer(uint imageIndex)
    {
        ...
        for (int j=0; j < 4; j++)
        {
            ...
            {
                Offset = new(-0.5f + frame * 0.0005f, -0.4f + j * 0.25f, 0.0f, 0.0f),
                Color = new(0.0f, 0.0f, 0.2f + 0.2f * j, 0.0f)
            };
            vk.CmdPushConstants(commandBuffers[imageIndex], pipelineLayout, ShaderStageFlags.VertexBit | ShaderStageFlags.FragmentBit, 0, SimplePushConstantData.SizeOf(), ref push);
