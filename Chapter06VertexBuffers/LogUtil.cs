...
#pragma warning restore CS8981 // The type name only contains lower-cased ascii characters. Such names may become reserved for the language.
{
    ...
    public static void RestartTimer()
    {
        ...
        TotalElapsed = 0;
        LastElapsed = 0;
        Console.WriteLine($"{"Memory",8} | {"Total",dt + 2} | {"Last",dt} | {"Lap",dt} | {"Name",-8} | Message");
        Console.WriteLine("--------------------------------------------------------------------------------------");
    }
    ...
    public static void d(string lap, string msg, bool skipTimer = false)
    {
        ...
        if (!skipTimer)
        {
            ...
            Console.WriteLine($"{memory,8} | {t2ms(now - FirstTick),dt + 2} | {t2ms(now - LastTick),dt} | {lapTicks,dt} | {lap,-8} | {msg}");
            LastTick = now;
        }
        else
        {
            Console.WriteLine($"{memory,8} | {string.Empty,dt + 2} | {string.Empty,dt} | {string.Empty,dt} | {string.Empty,-8} | {msg}");
        }
    }
